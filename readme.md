...
creates a virtual environment in ./.venv/, activates it, upgrades pip, and installs quads (along with its dependencies, declared in pyproject.toml) in editable mode. Installing it in editable mode allows the user to simply clone and run when a new version of quads comes (with out re-installation).
